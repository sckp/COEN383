...
void jobs_Round_Robin(Job* jobs, Job* finished_jobs, int numJobs) {
	...
	for(int i = 0; i < numJobs; i++) {
		...
		printf("ID: %i\tarrival time: %i\tservice time: %i\tpriority: %i\nremaining time: %i\tstart time: %i\tfinish time: %i\n\n",
			finished_jobs[i].pid, finished_jobs[i].arrival_time, finished_jobs[i].service_time, finished_jobs[i].priority,
			finished_jobs[i].remaining_service_time, finished_jobs[i].start_time, finished_jobs[i].finish_time);
	}
...
void processJobs(CPU* cpu, Queue* jobQueue, Job* jobs, Job* completed, int numJobs) {
	...
	// stop when the clock reaches 100
	while(150 > cpu_clock) {
		// check if a job has arrive and if it should be place into the queue
		if(cpu_clock == jobs[jobIndex].arrival_time) {
			
			printf("Pushing job %i on queue at cpu clock: %i\n", jobs[jobIndex].pid, cpu_clock);
